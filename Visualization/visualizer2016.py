...
def opener(url):
    ...
    pand['qt1'] = pand.date.apply(lambda e: datetime.datetime(2016,1,1) <= e < datetime.datetime(2016,4,1))
    pand['qt2'] = pand.date.apply(lambda e: datetime.datetime(2016,4,1) <= e < datetime.datetime(2016,7,1))
    pand['qt3'] = pand.date.apply(lambda e: datetime.datetime(2016,7,1) <= e < datetime.datetime(2016,10,1))
    pand['qt4'] = pand.date.apply(lambda e: datetime.datetime(2016,10,1) <= e < datetime.datetime(2017,1,1))
...
malitel = opener(r"C:\Users\bumso\datajournalism-2018\datajournalism-team-1\Beomsoo\navertv_json_data\2016\malitel_2016.json")
data_list = [(mudo,'mudo'), (nahonja,'nahonja'), (bokga,'bokga'), (radio,'radio'), (malitel, 'malitel')]
...
def bar_graph(data_list, category):
    ...
    plt.bar("mudo1", m[m.qt1][category].mean(), width=0.3)
    plt.bar("mudo2", m[m.qt2][category].mean(), width=0.3)
    plt.bar("mudo3", m[m.qt3][category].mean(), width=0.3)
    plt.bar("mudo4", m[m.qt4][category].mean(), width=0.3)
    ...
    plt.bar("malitel1", ma[m.qt1][category].mean(), width=0.3)
    plt.bar("malitel2", ma[m.qt2][category].mean(), width=0.3)
    plt.bar("malitel3", ma[m.qt3][category].mean(), width=0.3)
    plt.bar("malitel4", ma[m.qt4][category].mean(), width=0.3)
    plt.xticks(rotation = 90)
    plt.title(category + "_2016_bar")
    plt.savefig(category + "_2016_bar")   
    plt.show()
